...
class VPNWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.main_window = parent  # Store the main window reference
    ...
    def toggle_connection(self):
        ...
        if self.vpn_handler.is_connected:
            ...
            # Disable proxy in all browser tabs
            self._update_browser_proxy(False)
        else:
            try:
                ...
                if self.vpn_handler.connect(server):
                    ...
                    if ip_info:
                        ...
                        # Enable proxy in all browser tabs
                        self._update_browser_proxy(True, server['host'], server['port'])
                else:
    ...
    def _update_browser_proxy(self, enabled, host=None, port=None):
        """Update proxy settings in all browser tabs"""
        if self.main_window and hasattr(self.main_window, 'tabs_dict'):
            # Update proxy settings for all tabs
            for tab in self.main_window.tabs_dict.values():
                if hasattr(tab, 'update_proxy_settings'):
                    tab.update_proxy_settings(enabled, host, port)
            print(f"Updated proxy settings in all tabs: {'enabled' if enabled else 'disabled'}")
        else:
            print("Could not find main window to update proxy settings")
            # Print debug information
            print(f"Main window reference: {self.main_window}")
            if self.main_window:
                print(f"Main window attributes: {dir(self.main_window)}")
